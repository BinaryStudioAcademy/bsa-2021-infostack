...
export default class UserWorkspaceSeeder {
  public static async execute(): Promise<void> {
    ...
    await asyncForEach(async userWorkspace => {
      ...
      user_workspace.workspace = await workspaceRepository.findById(userWorkspace.workspaceId);
      let role;
      Object.values(UserRole).forEach(roleEnum => userWorkspace.role === roleEnum ? role = roleEnum : null);
