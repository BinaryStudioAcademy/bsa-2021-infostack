...
export default class TeamPermissionSeeder {
  public static async execute(): Promise<void> {
    ...
    await asyncForEach(async teamPermission => {
      ...
      team_permission.team = await teamRepository.findById(teamPermission.teamId);
      team_permission.page = await pageRepository.findById(teamPermission.pageId);
      let option;
      Object.values(PermissionOption).forEach(optionEnum => teamPermission.option === optionEnum ? option = optionEnum : null);
